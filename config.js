...
if (process.env.NODE_ENV == 'production') {
    ...
    const envConfig = dotenv.parse(fs.readFileSync('/var/veniqa/client-server/production/.env'))
    ...
    const envConfig = dotenv.parse(fs.readFileSync('/var/veniqa/client-server/development/.env'))
    for (let k in envConfig) {
        process.env[k] = envConfig[k]
    }
}
